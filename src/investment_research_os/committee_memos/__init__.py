...
class CommitteeMemo:
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "common_ground_statement_ids": list(
                self.common_ground_statement_ids
            ),
            "disagreement_records": [
                dict(item) for item in self.disagreement_records
            ],
            ...
            "state_disclosure": [
                item.as_dict() for item in self.state_disclosure
            ],
            "execution_metadata": {
...
class SynthesisAttempt:
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "result": (
                "validation_error" if validation_failed else "accepted"
            ),
            "validation_status": (
                "failed" if validation_failed else "passed"
            ),
            "validation_errors": list(self.validation_errors),
            "retry_reason": (
                self.validation_errors[0] if validation_failed else None
            ),
            "started_at": self.started_at.isoformat(),
...
class InMemoryCommitteeMemoRepository:
    def __init__(self) -> None:
        self._executions: dict[
            tuple[str, str], CommitteeMemoExecution
        ] = {}
    ...
    def save(self, execution: CommitteeMemoExecution) -> CommitteeMemoExecution:
...
class CommitteeMemoWorkflow:
    ...
    def execute(
        ...
        if (
            ...
            and evaluation_execution_identity_sha256(
                ...
                execution_contract_version=(
                    "committee-synthesis-contract-v1"
                ),
        ...
        logical_input = {
            ...
            "prompt": {
                ...
                "evaluation_corpus_id": (
                    request.prompt.evaluation_corpus_id
                ),
        ...
        for attempt_number in range(1, request.policy.max_attempts + 1):
            ...
            raw_payload_id, raw_hash = self._memo_repository.save_raw_attempt(
                operator.id,
                attempt_id,
                response.raw_output,
            )
            ...
            return self._memo_repository.save(execution)
...
def _validate_memo(
    ...
    return CommitteeMemo(
        ...
        executive_summary_statement_ids=categories[
            "executive_summary_statement_ids"
        ],
        statements=statements,
        common_ground_statement_ids=categories[
            "common_ground_statement_ids"
        ],
        ...
        evidence_gap_statement_ids=categories[
            "evidence_gap_statement_ids"
        ],
        invalidation_statement_ids=categories[
            "invalidation_statement_ids"
        ],
        ...
        provider_request_ids=tuple(
            item.provider_request_id for item in attempts
        ),
        attempts=attempts,
        usage=usage,
...
def _validate_disagreements(
    ...
    for value in values:
        ...
        if any(
            item is None or item.provenance_type != "gap"
...
def _memo_validation_code(error: CommitteeMemoError) -> str:
    return {
        "invalid memo provenance reference": (
        ...
        "invalid disagreement opinion reference": (
...
def _price_card_is_valid(
    ...
    if (
        ...
        or (
            ...
            and card.effective_to <= checked_at
        )
    ):
        return False
